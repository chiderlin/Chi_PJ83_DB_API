...
def C_data_DomainTestLog(request):
    ...
    if request.method == 'POST':
        ...
        data_serializer = DomainTestLogSerializer(data=data)
...
def R_data_DomainTestLog(request):
    ...
    if request.method == 'GET':
        domaintestlog = DomainTestLog.objects.all()
...
def U_data_DomainTestLog(request, id_):
    ...
    if request.method == 'PUT':
        domaintestlog = DomainTestLog.objects.get(id=id_)
        data = JSONParser().parse(request)
...
def D_all_data_DomainTestLog(request):
    ''' delete all. ''' 
    if request.method == 'DELETE':
        count = DomainTestLog.objects.all().delete()
        # print(count)
...
def C_data_DomainListAll(request):
    ...
    if request.method == 'POST':
        data = JSONParser().parse(request)
        data_serializer = DomainListAllSerializer(data=data)
        if data_serializer.is_valid():
            data_serializer.save()
            return JsonResponse(data_serializer.data, status=status.HTTP_201_CREATED)
        return JsonResponse(data_serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
def R_data_DomainListAll(request):
    '''show all of the data.'''
    if request.method == 'GET':
        domainlistall = DomainListAll.objects.all()
...
@api_view(['PUT'])
def U_data_DomainListAll(request, codetomatch=None):
    ''' update specific data using codetomatch. '''
    if request.method == 'PUT':
        domainlistall = DomainListAll.objects.get(CodeToMatch=codetomatch)
        data = JSONParser().parse(request)
...
@api_view(['DELETE'])
def D_data_DomainListAll(request, codetomatch=None):
    ''' delete specific data using codetomatch. '''
    if request.method == 'DELETE':
        if codetomatch != None:
            domainlistall = DomainListAll.objects.get(CodeToMatch=codetomatch)
            domainlistall.delete() 
            return JsonResponse({'message': f'CodeToMatch {codetomatch} was deleted successfully!'}, status=status.HTTP_204_NO_CONTENT)
...
def D_all_data_DomainListAll(request):
    ''' delete all. ''' 
    if request.method == 'DELETE':
        count = DomainListAll.objects.all().delete()
        # print(count)
